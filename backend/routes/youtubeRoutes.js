...
router.get("/auth-url", protect, checkSuspension, async (req, res) => {
  try {
...
router.get("/oauth2callback", async (req, res) => {
  ...
  if (error) {
    ...
    return res.redirect(
      `${process.env.FRONTEND_URL}/register/influencer?error=auth_error`
    );
  }

  if (!code) {
    return res.redirect(
      `${process.env.FRONTEND_URL}/register/influencer?error=missing_code`
    );
  }
  ...
  try {
    ...
    const channelStats = {
      ...
      averageViews:
        ...
        (parseInt(channel.statistics.videoCount || 1) || 1),
    ...
    const [
      ...
      youtubeAnalytics.reports
        .query({
          ...
          metrics:
            "estimatedMinutesWatched,averageViewDuration,averageViewPercentage,views",
        })
    ...
    if (demographicsData?.data?.rows) {
      demographicsData.data.rows.forEach((row) => {
        ...
        if (gender === "male")
          genderDistribution.male += parseFloat(percentage) || 0;
        else if (gender === "female")
          genderDistribution.female += parseFloat(percentage) || 0;
    ...
    try {
      if (
        geographicData?.data?.rows &&
        Array.isArray(geographicData.data.rows) &&
        geographicData.data.rows.length > 0
      ) {
    ...
    try {
      ...
      if (videoIds.length > 0) {
        try {
          videoStats = await youtube.videos
            .list({
              part: "statistics",
              id: videoIds.join(","),
            })
      ...
      if (topVideos?.data?.items && Array.isArray(topVideos.data.items)) {
        topPerformingVideos = topVideos.data.items
          .map((video, index) => {
            try {
    ...
    try {
      if (basicMetrics?.data?.rows?.[0]) {
        const [watchTime, viewDuration, viewPercentage, views] =
          basicMetrics.data.rows[0].map((val) => parseFloat(val) || 0);
      ...
      if (subscriberGrowth?.data?.rows?.[0]?.[0]) {
        const subscribersGained =
          parseFloat(subscriberGrowth.data.rows[0][0]) || 0;
    ...
    res.redirect(
      `${process.env.FRONTEND_URL}/register/influencer?youtubeData=${encodedData}&step=3`
    );
  } catch (error) {
    console.error("YouTube API Error:", error);
    res.redirect(
      `${process.env.FRONTEND_URL}/register/influencer?error=server_error`
    );
  }
});

// Add a route to refresh YouTube data (protected)
router.post("/refresh-data", protect, checkSuspension, async (req, res) => {
  try {
    ...
    if (!influencer || !influencer.youtube.oauthTokens) {
      return res
        .status(404)
        .json({ message: "Influencer or OAuth tokens not found" });
    }
    ...
    if (demographicsData.data.rows) {
      demographicsData.data.rows.forEach((row) => {
        const [age, gender, percentage] = row;
        if (gender === "male")
          genderDistribution.male += parseFloat(percentage);
        else if (gender === "female")
          genderDistribution.female += parseFloat(percentage);
    ...
    const topCountries =
      geographicData.data.rows?.map((row) => ({
        country: row[0],
    ...
    influencer.youtube = {
      ...
      averageViews:
        ...
        (parseInt(channel.statistics.videoCount) || 1),
      demographics: {
        ageGroups,
        genderDistribution,
        topCountries,
      ...
      analytics: {
        ...
        subscribersGainedLast30Days: parseInt(
          subscriberGrowth.data.rows[0][0] / 3
        ),
        ...
        averageWatchTime: parseInt(
          basicMetrics.data.rows[0][0] / basicMetrics.data.rows[0][3]
        ),
